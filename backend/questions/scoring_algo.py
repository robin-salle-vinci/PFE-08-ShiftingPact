...
def calculate_theme_scores(challenge_scores) -> Dict[str, Dict[str, float]]:
    ...
    for theme, scores in theme_scores["in_two_years"].items():
        ...
        theme_scores["in_two_years"][theme]["percentage"] = (scores["score"] / in_two_years_max * 100) if in_two_years_max > 0 else 0.0
...
def calculate_global_esg_scores(theme_scores) -> Dict[str, float]:
    try:
        ...
        total_score_in_two_years = sum(theme["percentage"] for theme in theme_scores["in_two_years"].values()) if \
        theme_scores["in_two_years"] else 0.0
